//! CLOSED-LOOP STAGE 1 (OBSERVE-ONLY) scope. Copy of scope1.rs (valley sampling)
//! plus a real-time BEMF zero-crossing detector that logs, per commutation, what a
//! closed-loop controller WOULD see. It STEERS NOTHING — commutation stays 100%
//! open-loop. Per CLOSED_LOOP_PLAN.md: the Stage-1 gate is that this real-time ZC
//! matches the OFFLINE multi-harmonic oracle on the same captured frames; only then
//! is feedback (a later file/stage) allowed. The detector params (CL_BLANK/CL_CONFIRM)
//! are INITIAL values to be calibrated against the oracle.
//!
...
const STEPS_PER_PHYS_SECTOR: u32 = ELEC_STEPS_PER_REV / 6; // 24
const SIX_HIGH: [usize; 6] = [0, 0, 1, 1, 2, 2];
const SIX_LOW: [usize; 6] = [1, 2, 2, 0, 0, 1];
const CL_BLANK: u32 = 2; // skip first N frames of a sector (commutation/demag) — CALIBRATE
const CL_CONFIRM: u32 = 2; // consecutive opposite-sign samples to confirm a ZC — CALIBRATE
...
fn main() -> ! {
    ...
    writeln!(
        tx,
        "scope_cl ready (CL stage1 OBSERVE-ONLY, steers nothing)  f/v=±10Hz g/b=±1Hz a/z=±1% +/-=±0.1% [/]=±1ct w=off d=dump l/k=stream q=reset\r"
    )
    .ok();
...
fn dump_cl_log<TX: Write>(tx: &mut TX) {
    ...
    writeln!(
        tx,
        "cl: {} commutations OBSERVE-ONLY (i phys zc_pct period_ticks)\r",
        n
    )
    .ok();
    for i in 0..n {
        ...
        writeln!(tx, "cl i={} phys={} zc={} per={}\r", i, phys, zc, per).ok();
    }
}
...
extern "C" fn TIM7() {
    ...
    unsafe {
        ...
        // ---- OBSERVE-ONLY closed-loop ZC detector + per-commutation log ----
        // Detects the BEMF zero crossing in real time on the valley sample and, during
        // a capture window, records per commutation what a controller WOULD see. STEERS
        // NOTHING — commutation above is 100% open-loop. Records only during capture so
        // the log aligns 1:1 with the dumped frames (the oracle reads the same frames).
        {
            static mut CL_PHYS_PREV: u32 = 99;
            static mut CL_FRAME: u32 = 0; // frames since this sector began
            static mut CL_TICKS: u32 = 0; // ticks since this sector began (period proxy)
            static mut CL_FOUND: bool = false;
            static mut CL_PCT: u32 = 255;
            static mut CL_SIGN: i32 = 0; // established entry sign of (float - neutral)
            static mut CL_CONF: u32 = 0;

            let phys = (STEP / STEPS_PER_PHYS_SECTOR) % 6;
            let fps = (DRIVE_HZ / (electrical_hz.max(1) * 6)).max(1);
            if phys != CL_PHYS_PREV {
                // commutation boundary: finalize the just-ended sector
                if CL_PHYS_PREV != 99 && CAPTURE_ACTIVE {
                    let n = CL_CAP_N.load(Ordering::Relaxed) as usize;
                    if n < CL_MAXCOMM {
                        let zcb = if CL_FOUND { CL_PCT.min(100) } else { 255 };
                        let perb = CL_TICKS.min(255);
                        CL_CAP[n]
                            .store((CL_PHYS_PREV << 16) | (zcb << 8) | perb, Ordering::Relaxed);
            ...
            let hi = SIX_HIGH[phys as usize];
            let lo = SIX_LOW[phys as usize];
            let fl = 3 - hi - lo;
            let neutral = (frame[hi] as i32 + frame[lo] as i32) / 2;
            let e = frame[fl] as i32 - neutral;
            let sign = if e > 0 {
                1
            } else if e < 0 {
                -1
            ...
            if CL_FRAME > CL_BLANK && !CL_FOUND {
                if CL_SIGN == 0 {
                    CL_SIGN = sign; // establish post-demag entry sign
                } else if sign != 0 && sign != CL_SIGN {
                    CL_CONF += 1;
                    if CL_CONF >= CL_CONFIRM {
                        CL_FOUND = true;
                        CL_PCT = (CL_FRAME * 100) / fps; // ZC position in the 60-deg window
                    }
                } else if sign == CL_SIGN {
                    CL_CONF = 0; // back on the entry side -> not a confirmed crossing
                }
            }
        }
    }
