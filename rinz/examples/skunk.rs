...
//!   y       toggle closed-loop ZC-hunting (variable commutation driven by BEMF)
//!   q       reset to defaults and run
//!
//! Closed-loop ('y') — a deliberately basic "lay the railroad" handoff:
//!   * State 1 (open loop) is whatever drive is already running at ELECTRICAL_HZ.
//!   * On engage, the ISR seeds a commutation period (ticks) from that frequency and
//!     takes over the six-step outputs as 6 coarse physical states (no micro-step).
//!   * Each state: blank the demag tail (CL_BLANK_TICKS), then watch the floating
//!     phase cross the (Va+Vb+Vc)/3 virtual neutral. CL_ZC_CONFIRM consecutive
//!     samples on the far side = a confirmed zero-cross at tick T_zc.
//!   * Schoolbook: ZC sits at 30 deg of the 60 deg sector, so sector length = 2*T_zc.
//!     We don't trust raw math — a ZC is only accepted inside a plausible window
//!     (CL_TRUST_*), the new period is slew-clamped (CL_SLEW_DIV) so the rotor can't
//!     claim an impossible jump, and it's IIR-blended (CL_IIR_*) into the running
//!     period as a soft handoff. If no trustworthy ZC arrives, the old period just
//!     commutates open-loop — it free-wheels instead of falling over.
//!   All limits are compile-time consts so the ISR path stays short.
...
// --- 'y' closed-loop ZC-hunting (basic) ---------------------------------------
// All baked in at compile time so the high-rate ISR path stays a few instructions.
// Floating phase per physical six-step state (0=A,1=B,2=C). State s drives logical
// sector s*12; see DRIVE_* — the float slot rotates C,B,A,C,B,A.
const CL_FLOAT_IDX: [usize; 6] = [2, 1, 0, 2, 1, 0];
// Ignore this many ticks after a commutation (the demagnetization cliff). This is
// an ABSOLUTE floor because the demag tail is roughly fixed in wall-clock time
// (~tick 6 = ~300 us at 20 kHz on this motor, measured: the loop collapsed to
// cl_per=12 = 2x6 when this was too small). It MUST stay above the demag crossing
// or the loop locks onto demag instead of BEMF and the period death-spirals down.
// Trade-off: it also caps the top speed (blank must be < half the sector), so raise
// it only as far as needed to clear demag. Tunable knob #1 if it still mis-locks.
// A/B history (commanded 120 Hz, seed period 27): blank 8 -> cl_per 22, blank 10 ->
// cl_per 26. The detected ZC tick tracked the blank (cl_per ~= 2*(blank+3)), i.e. the
// lock is slaved to "~3 ticks after the blank lifts," NOT a fixed physical crossing.
// At 12 the artifact predicts cl_per ~30 (cl_hz ~111); a genuine fixed ZC would hold
// cl_per ~26. This is the discriminator. (Root issue: the sector is too short at
// 120 Hz to separate demag/blank from the real ZC. Run slower for a roomier window.)
const CL_BLANK_TICKS: u32 = 12;
// Consecutive samples past neutral required to call a real edge (PWM-ring debounce).
const CL_ZC_CONFIRM: u32 = 2;
// "Window of trust": accept a ZC only if it lands between 0.3 and 0.7 of the current
// period (ideal is 0.5). Anything earlier/later is treated as noise and dropped.
const CL_TRUST_LO_X10: u32 = 3;
const CL_TRUST_HI_X10: u32 = 7;
// Slew limit: the period may move by at most period/CL_SLEW_DIV per sector, so the
// rotor can't dictate a physically impossible acceleration.
const CL_SLEW_DIV: u32 = 8;
// Soft-handoff IIR: new_period = (NUM*raw + (DEN-NUM)*old) / DEN. Small NUM = gentle.
const CL_IIR_NUM: u32 = 2;
const CL_IIR_DEN: u32 = 8;
// Safety rails on the period (ticks) regardless of what the math proposes.
const CL_PERIOD_MIN: u32 = 4;
const CL_PERIOD_MAX: u32 = 20_000;
...
// 'y' closed-loop ZC-hunting: engaged flag + telemetry surfaced on the debug line.
static CLOSED_LOOP: AtomicBool = AtomicBool::new(false);
static DBG_CL_PERIOD: AtomicU32 = AtomicU32::new(0);
static DBG_CL_LOCKS: AtomicU32 = AtomicU32::new(0);
// Direct ZC diagnostics: tick of the last accepted crossing, and the (float-neutral)
// excursion across the last sector's post-blank window. The span answers "is there a
// resolvable crossing here?" without inferring it from the period.
static DBG_CL_TZC: AtomicU32 = AtomicU32::new(0);
static DBG_CL_VFL_LO: AtomicI32 = AtomicI32::new(0);
static DBG_CL_VFL_HI: AtomicI32 = AtomicI32::new(0);

/// -1 / 0 / +1 sign of an i32, branch-free.
#[inline]
fn isign(x: i32) -> i32 {
    ...
    C3: SetDutyCycle,
{
    match cmd {
        ...
        b'y' => {
            let on = !CLOSED_LOOP.load(Ordering::Relaxed);
            CLOSED_LOOP.store(on, Ordering::Relaxed);
            DBG_CL_LOCKS.store(0, Ordering::Relaxed);
            writeln!(tx, "closed-loop={}\r", if on { "on" } else { "off" }).ok();
        }
        b'w' => {
            CLOSED_LOOP.store(false, Ordering::Relaxed);
...
fn main() -> ! {
    ...
    writeln!(
        tx,
        "skunk ready  f/v=±10Hz g/b=±1Hz a/z=±1% +/-=±0.1% [/]=±1ct w=off d=dump l/k=stream y=closed-loop q=reset\r"
    )
    .ok();
...
fn run_capture<TX: Write>(
    ...
    // Closed-loop's actual locked electrical frequency from the filtered period:
    // 6 commutations per electrical rev, DRIVE_HZ ticks/sec. 0 if not locked yet.
    let cl_per = DBG_CL_PERIOD.load(Ordering::Relaxed);
    let cl_hz = if cl_per > 0 {
        DRIVE_HZ / (6 * cl_per)
    } else {
        0
    };
    writeln!(
        tx,
        "debug: hz={} amp={} trim={} vbus_mv={} iu_ma={} tim7={} six_ticks={} dma_tc={} dma_ht={} dma_te={} cl={} cl_per={} cl_hz={} cl_locks={} t_zc={} vfl_lo={} vfl_hi={}\r",
        ...
        CLOSED_LOOP.load(Ordering::Relaxed) as u32,
        cl_per,
        cl_hz,
        DBG_CL_LOCKS.load(Ordering::Relaxed),
        DBG_CL_TZC.load(Ordering::Relaxed),
        DBG_CL_VFL_LO.load(Ordering::Relaxed),
        DBG_CL_VFL_HI.load(Ordering::Relaxed)
    )
    .ok();
...
extern "C" fn TIM7() {
    ...
    // Closed-loop ZC-hunting state (only live while CLOSED_LOOP is engaged).
    static mut CL_PREV: bool = false; // engaged last tick? detect the rising edge to seed
    static mut CL_STATE: u32 = 0; // current physical six-step state 0..5
    static mut CL_TICKS: u32 = 0; // ticks since this state's commutation
    static mut CL_PERIOD: u32 = 64; // current (filtered) sector length in ticks
    static mut CL_ZC_DONE: bool = false; // already resolved a ZC this sector?
    static mut CL_CONFIRM: u32 = 0; // consecutive far-side samples
    static mut CL_INIT_SIGN: i32 = 0; // sign of (float - neutral) just after blanking
    static mut CL_VMIN: i32 = i32::MAX; // running min/max of (float-neutral) this sector
    static mut CL_VMAX: i32 = i32::MIN;
    ...
    unsafe {
        ...
        // The open-loop STEP accounting above keeps running in either mode so the
        // 'd'/'l' capture still triggers on its nominal electrical zero. Only the
        // actual phase outputs change owner when closed-loop is engaged.
        if CLOSED_LOOP.load(Ordering::Relaxed) {
            // --- State seed on the engage edge: start from where open loop was, and
            // derive the sector length from the current commanded frequency. 6 physical
            // commutations per electrical revolution.
            if !CL_PREV {
                CL_STATE = (STEP / (ELEC_STEPS_PER_REV / 6)) % 6;
                CL_PERIOD =
                    (DRIVE_HZ / (6 * electrical_hz.max(1))).clamp(CL_PERIOD_MIN, CL_PERIOD_MAX);
                CL_TICKS = 0;
                CL_ZC_DONE = false;
                CL_CONFIRM = 0;
                CL_INIT_SIGN = 0;
            ...
            // Blank past the demagnetization tail. The demag edge sits at a roughly
            // FIXED time after commutation, so a fixed tick count lets it slip back
            // inside the window once the period shrinks — and the loop then locks onto
            // demag instead of the real ZC (period collapses to ~2x the demag time).
            // Tie the blank to the period: never look before the trust window opens
            // (0.3*period) anyway, with CL_BLANK_TICKS as an absolute floor.
            let blank = CL_BLANK_TICKS.max(CL_PERIOD * CL_TRUST_LO_X10 / 10);

            // --- Read the most recent complete ADC frame straight from the live DMA
            // buffer (MAR/NDTR locate it regardless of which capture buffer is active).
            // Runs every post-blank tick (even after a ZC is resolved) so the excursion
            // diagnostics see the whole window; only ZC *detection* stops once resolved.
            if CL_TICKS > blank {
                let dma = &*stm32::DMA1::ptr();
                let chd = dma.ch1();
                let dbase = chd.mar().read().bits() as *const u16;
                let ndtr = chd.ndtr().read().ndt().bits() as usize;
                let complete = ADC_BUF_LEN.saturating_sub(ndtr) / ADC_CHANNELS;
                if complete >= 1 {
                    let i = (complete - 1) * ADC_CHANNELS;
                    let a = core::ptr::read_volatile(dbase.add(i)) as i32;
                    let b = core::ptr::read_volatile(dbase.add(i + 1)) as i32;
                    let c = core::ptr::read_volatile(dbase.add(i + 2)) as i32;
                    let vn = (a + b + c) / 3;
                    let v_fl = match CL_FLOAT_IDX[CL_STATE as usize] {
                        0 => a,
                        1 => b,
                        _ => c,
                    };
                    let diff = v_fl - vn;
                    // Diagnostic excursion across the whole post-blank window: tells us
                    // whether the float phase actually swings through neutral (a real,
                    // resolvable ZC) or just hovers in the noise.
                    if diff < CL_VMIN {
                        CL_VMIN = diff;
                    }
                    if diff > CL_VMAX {
                    ...
                    // ZC detection — only until resolved this sector.
                    if !CL_ZC_DONE {
                        let s = isign(diff);
                        if CL_TICKS == blank + 1 {
                            // First post-blank sample fixes which side we start on.
                            CL_INIT_SIGN = s;
                            CL_CONFIRM = 0;
                        } else if CL_INIT_SIGN != 0 && s != 0 && s != CL_INIT_SIGN {
                            CL_CONFIRM += 1;
                            if CL_CONFIRM >= CL_ZC_CONFIRM {
                                let t_zc = CL_TICKS;
                                // Window of trust: only believe a ZC near sector midpoint.
                                let lo = CL_PERIOD * CL_TRUST_LO_X10 / 10;
                                let hi = CL_PERIOD * CL_TRUST_HI_X10 / 10;
                                if t_zc >= lo && t_zc <= hi {
                                    // Schoolbook: full sector = 2*T_zc. Slew-clamp then
                                    // IIR-blend it in (soft handoff).
                                    let lim = (CL_PERIOD / CL_SLEW_DIV).max(1);
                                    let raw = (2 * t_zc).clamp(CL_PERIOD - lim, CL_PERIOD + lim);
                                    CL_PERIOD = ((CL_IIR_NUM * raw
                                        + (CL_IIR_DEN - CL_IIR_NUM) * CL_PERIOD)
                                        / CL_IIR_DEN)
                                        .clamp(CL_PERIOD_MIN, CL_PERIOD_MAX);
                                    DBG_CL_TZC.store(t_zc, Ordering::Relaxed);
                                    DBG_CL_LOCKS.fetch_add(1, Ordering::Relaxed);
                                }
                                // Accepted or rejected, stop hunting this sector; if it
                                // was rejected the old period just commutates open-loop.
                                CL_ZC_DONE = true;
                            }
                        } else {
                            CL_CONFIRM = 0;
            ...
            // --- Commutate when the (filtered) sector length elapses. Free-wheels on
            // the carried-over period when no trustworthy ZC was found.
            if CL_TICKS >= CL_PERIOD {
                // Latch the just-ended sector's float-neutral excursion for the dump.
                if CL_VMAX >= CL_VMIN {
                    DBG_CL_VFL_LO.store(CL_VMIN, Ordering::Relaxed);
                    DBG_CL_VFL_HI.store(CL_VMAX, Ordering::Relaxed);
                }
                CL_VMIN = i32::MAX;
                CL_VMAX = i32::MIN;
                CL_STATE = (CL_STATE + 1) % 6;
                CL_TICKS = 0;
                CL_ZC_DONE = false;
                CL_CONFIRM = 0;
                CL_INIT_SIGN = 0;
            }
            DBG_CL_PERIOD.store(CL_PERIOD, Ordering::Relaxed);
            set_six_step((CL_STATE * 12) as u8, duty);
        } else {
            CL_PREV = false;
