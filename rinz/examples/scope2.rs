//! DUAL peak+valley BEMF scope: samples TWICE per PWM period (40 kHz), at both the
//! valley (CNT=0) and the peak (CNT=ARR). Derived from scope1.rs (valley-only).
//!
//! In center-aligned mode the valley and peak are fixed timer instants regardless
//! of duty (duty only moves the CCR edges between them). With RCR=0 the TIM1 UPDATE
//! event fires at BOTH underflow (valley) and overflow (peak), so routing
//! update→TRGO scans the ADC at two duty-independent instants per period:
//!   valley (CNT=0):  driven pair {Vbus, GND}, neutral ~Vbus/2 → FLOAT reads full
//!                    bipolar BEMF (the scope1 signal).
//!   peak   (CNT=ARR): driven pair both GND,    neutral ~0      → FLOAT reads BEMF
//!                    vs GND (positive half; negative clamps at the ADC floor).
//! The two are half a period (25 us) apart → a uniform 40 kHz interleaved stream.
//! Peak/valley frames are NOT tagged: the host tells them apart by driven-channel
//! level (valley = one driven phase ~Vbus; peak = both driven ~GND). See
//! DUAL_SAMPLE_PLAN.md for the rationale, host steps, and validation.
//!
//! CR2.MMS=0b010 (update) → TRGO → ADC1/ADC2 EXTSEL=Tim_1_trgo. 12-bit samples,
//! 40 kHz frame rate (1 frame = ch17/PA4, ch5/PC4, ch14/PB11 BEMF voltages +
...
//!           (interleaved peak+valley frames at 40 kHz)
...
// TIM1_TRGO(update)-triggered ADC2 scan of the three BEMF phases:
// ch17/PA4, ch5/PC4, ch14/PB11 — TWO scans per PWM period (valley CNT=0 + peak
// CNT=ARR). The valley timing budget below is the tighter of the two at high duty.
...
// 2 scans per PWM period: valley (CNT=0) AND peak (CNT=ARR), via TIM1 update→TRGO.
const ADC_SAMPLES_PER_PWM: u32 = 2;
const ADC_FRAME_HZ: u32 = PWM_HZ * ADC_SAMPLES_PER_PWM; // 40 kHz
// Buffer must hold this many revolutions at the lowest fully-supported freq. At the
// doubled 40 kHz frame rate, holding the same SRAM as scope1 means 2 full revs down
// to ~120 Hz; below that the capture is buffer-bound (<2 revs — fine, dual-rate is
// for high RPM, and low RPM already has abundant samples).
const CAPTURE_REVS: u32 = 2;
const CAPTURE_MIN_HZ: u32 = 120;
...
/// Route TIM1 UPDATE to ADC TRGO — one ADC scan at BOTH the valley (CNT=0,
/// underflow) and the peak (CNT=ARR, overflow) of every PWM period.
///
/// In center-aligned mode with RCR=0 the update event fires at both over- and
/// underflow, so the ADC samples twice per period (40 kHz) at two duty-independent
/// instants. Peak/valley are not tagged in the stream; the host tells them apart by
/// the driven-channel level (a valley frame has one driven phase ~Vbus; a peak frame
/// has both driven phases ~GND — the driven-pair neutral then comes out ~Vbus/2 and
/// ~0 respectively, automatically).
fn configure_adc_peakvalley_trgo() {
    let t1 = unsafe { &*stm32::TIM1::ptr() };
    // RCR=0: update event at every over- AND underflow (2 UEV per period). Forced
    // explicitly so the trigger rate doesn't depend on what the HAL left in RCR.
    // Harmless to the PWM: ARR is constant and CCR is updated slowly from the ISR.
    t1.rcr().write(|w| unsafe { w.bits(0) });
    // CR2.MMS = 0b010 (update event → TRGO). (was 0b111 OC4REF for valley-only.)
    t1.cr2()
        .modify(|r, w| unsafe { w.bits((r.bits() & !0x70) | (0b010u32 << 4)) });
}

/// TIM7 ticks for one capture: CAPTURE_REVS electrical revs, but capped so the ADC
/// (ADC_SAMPLES_PER_PWM frames per tick) can't overrun the DMA buffer. Below the
/// design-min freq this yields <CAPTURE_REVS revs (buffer-bound).
fn capture_tick_count(electrical_hz: u32) -> u32 {
    let hz = electrical_hz.max(1);
    let by_revs = (DRIVE_HZ * CAPTURE_REVS).div_ceil(hz);
    let by_buf = (ADC_FRAME_COUNT as u32) / ADC_SAMPLES_PER_PWM;
    by_revs.min(by_buf).max(1)
}

/// ADC frames produced over `ticks` TIM7 ticks — exactly ADC_SAMPLES_PER_PWM per
/// tick: TIM1 UEV (40 kHz) and TIM7 (20 kHz) are both integer divides of the same
/// 170 MHz clock, so they are 2:1 locked and `frames = ticks * 2` is exact (≤ buffer
/// by capture_tick_count's cap → never wraps the circular DMA).
fn capture_frame_count(ticks: u32) -> u32 {
    ticks * ADC_SAMPLES_PER_PWM
}
...
fn main() -> ! {
    ...
    writeln!(
        tx,
        "scope2 ready (dual peak+valley, 40kHz)  f/v=±10Hz g/b=±1Hz a/z=±1% +/-=±0.1% [/]=±1ct w=off d=dump l/k=stream q=reset\r"
    )
    .ok();
...
fn dump_buffer<TX: Write>(tx: &mut TX, ptr: *const u16, ptr1: *const u16, frames: usize) {
    writeln!(
        tx,
        "dump7: {} frames x 7 channels (ch17 ch5 ch14 ch16 ch18 ch13 ch1, 12-bit ADC, interleaved peak+valley, {} Hz)\r",
        frames, ADC_FRAME_HZ
    )
    .ok();
...
fn dump_buffer_b85<TX: Write>(tx: &mut TX, ptr: *const u16, ptr1: *const u16, frames: usize) {
    writeln!(
        tx,
        "cdump: {} frames x 7 channels (ch17 ch5 ch14 ch16 ch18 ch13 ch1, 12-bit, b85, interleaved peak+valley, {} Hz)\r",
        frames, ADC_FRAME_HZ
    )
    .ok();
    let mut group = [0u8; 4];
...
extern "C" fn TIM7() {
    ...
    unsafe {
        ...
        if CAPTURE_WAIT_ZERO && wrapped_to_zero {
            let ticks = capture_tick_count(electrical_hz);
            let frames = capture_frame_count(ticks);
