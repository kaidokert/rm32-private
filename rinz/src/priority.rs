//! NVIC + SCB priority setup for the rinz bench prototypes.
//!
//! Modeled after `ref/usb-servo-rs/nuc412/src/priority.rs`, adapted for
//! STM32G431 (Cortex-M4, `__NVIC_PRIO_BITS = 4` — same 4-bit upper-nibble
//! encoding as F4).
//!
...
//! | 1   | COMP1_2_3      | BEMF zero-cross — short, latency-critical          |
...
/// IPR byte / SCB SHPR byte. STM32G4 only implements the top 4 bits.
#[inline(always)]
...
pub unsafe fn set_irq_prios() {
    unsafe {
        ...
        set_irq_prio(Interrupt::COMP1_2_3, PRIO_COMP);
...
pub fn dump_irq_prios() {
    let comp = NVIC::get_priority(Interrupt::COMP1_2_3);
    ...
    rprintln!("NVIC COMP1_2_3   = 0x{:02X} (expect 0x10)", comp);
