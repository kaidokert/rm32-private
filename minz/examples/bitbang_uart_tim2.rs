...
fn EXTI0() {
    free(|cs| {
        ...
        if UART.borrow(cs).borrow_mut().on_falling_edge(ticks_10us()) {
            // Open the gate; TIM2 ISR (always running) will start feeding
            // samples into the decoder. Sample phase relative to the start
            // edge is uncontrolled, but at OVERSAMPLE=4 all four samples
            // per bit still land inside the bit so majority vote is fine.
            hw.rx_active = true;
        }
    });
}
